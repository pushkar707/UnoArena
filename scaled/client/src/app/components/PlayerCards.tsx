...
const PlayerCards = ({ positionId, orientation, classes='' }: {positionId: number, orientation: 'horizontal' | 'vertical', classes?:string }) => {
    ...
    return (
        <div className={`${classes} flex flex-col justify-center items-center gap-y-1.5`}>
            ...
            <p className={`text-center font-medium text-lg text-black mt-1.5 max-w-[80px] md:max-w-[145px] xl:max-w-max ${gameData.nextTurn === positionId && 'font-semibold underline'}`}>{gameData.players.find((pl: any) => pl.id === positionId)?.name}</p>
        </div>
    )
}
