  ...
  return (
    showJoinRoomPopup ? <div className='absolute w-screen h-screen top-0 left-0 z-10 bg-gray-200 bg-opacity-90 flex justify-center items-center'>
      <form onSubmit={joinRoom} className='bg-white px-8 py-6 rounded-md shadow relative pt-9'>
        <span className="absolute top-1 right-2 text-lg font-medium cursor-pointer text-black" onClick={() => setshowJoinRoomPopup(false)}>X</span>
        <input type="text" required placeholder="Enter your name" value={roomJoineeName} onChange={e => setRoomJoineeName(e.target.value)} className="px-3 py-2 border text-black border-black" />
